/*
 *  drivers/cpufreq/cpufreq_neokdemand2.c
 *
...
#define DEF_SAMPLING_DOWN_MAX_MOMENTUM		(18)
#define DEF_SAMPLING_DOWN_MOMENTUM_SENSITIVITY	(90)
#define MAX_SAMPLING_DOWN_MOMENTUM_SENSITIVITY	(1000)
#define MICRO_FREQUENCY_DOWN_DIFFERENTIAL	(8)
#define MICRO_FREQUENCY_UP_THRESHOLD		(95)
#define MICRO_FREQUENCY_MIN_SAMPLE_RATE		(97000)
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_neokdemand2
static
#endif
struct cpufreq_governor cpufreq_gov_neokdemand2 = {
       .name                   = "neokdemand2",
...
static struct workqueue_struct	*kneokdemand2_wq;
...
static void neokdemand2_powersave_bias_init_cpu(int cpu)
{
...
static void neokdemand2_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		neokdemand2_powersave_bias_init_cpu(i);
	}
}
...
{
	printk_once(KERN_INFO "CPUFREQ: neokdemand2 sampling_rate_max "
...
/* cpufreq_neokdemand2 Governor Tunables */
...
#define DEPRECATION_MSG(file_name)					\
	printk_once(KERN_INFO "CPUFREQ: Per core neokdemand2 sysfs "	\
				    ...
				    const char *buf, size_t count)
{
	...
	neokdemand2_powersave_bias_init();
	mutex_unlock(&dbs_mutex);
...
*/
		...
		/*
		 * For the purpose of neokdemand2, waiting for disk IO is an
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, kneokdemand2_wq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, kneokdemand2_wq, &dbs_info->work,
		delay);
}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		neokdemand2_powersave_bias_init_cpu(cpu);
		/*
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	kneokdemand2_wq = create_workqueue("kneokdemand2");
	if (!kneokdemand2_wq) {
		printk(KERN_ERR "Creation of kneokdemand2 failed\n");
		return -EFAULT;
	}
	err = cpufreq_register_governor(&cpufreq_gov_neokdemand2);
	if (err)
		destroy_workqueue(kneokdemand2_wq);
...
{
	cpufreq_unregister_governor(&cpufreq_gov_neokdemand2);
	destroy_workqueue(kneokdemand2_wq);
}
...
MODULE_DESCRIPTION("'cpufreq_neokdemand2' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_neokdemand2
